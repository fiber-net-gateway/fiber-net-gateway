...
public final class Snappy {
    ...
    public void decode(ByteBuf in, ByteBuf out) throws DataCodecException {
        while (in.isReadable()) {
            switch (state) {
     ...
     */
    private static int readPreamble(ByteBuf in) throws DataCodecException {
        int length = 0;
        int byteIndex = 0;
        while (in.isReadable()) {
            ...
            if (byteIndex >= 4) {
                throw new DataCodecException("Preamble is greater than 4 bytes");
            }
        }
     ...
     * no preamble is able to be calculated
     */
    int getPreamble(ByteBuf in) throws DataCodecException {
     ...
     * "try again later"
     * @throws DataCodecException If the read offset is invalid
     */
    private static int decodeCopyWith1ByteOffset(byte tag, ByteBuf in, ByteBuf out, int writtenSoFar) throws DataCodecException {
        if (!in.isReadable()) {
            return NOT_ENOUGH_INPUT;
        }
     ...
     * @throws DataCodecException If the read offset is invalid
     */
    private static int decodeCopyWith2ByteOffset(byte tag, ByteBuf in, ByteBuf out, int writtenSoFar) throws DataCodecException {
        if (in.readableBytes() < 2) {
            return NOT_ENOUGH_INPUT;
        }
     ...
     * @throws DataCodecException If the read offset is invalid
     */
    private static int decodeCopyWith4ByteOffset(byte tag, ByteBuf in, ByteBuf out, int writtenSoFar) throws DataCodecException {
        if (in.readableBytes() < 4) {
            return NOT_ENOUGH_INPUT;
        }
     ...
     * @throws DataCodecException if the offset is invalid
     */
    private static void validateOffset(int offset, int chunkSizeSoFar) throws DataCodecException {
        if (offset == 0) {
            throw new DataCodecException("Offset is less than minimum permissible value");
        }

        if (offset < 0) {
            // Due to arithmetic overflow
            throw new DataCodecException("Offset is greater than maximum value supported by this implementation");
        }

        if (offset > chunkSizeSoFar) {
            throw new DataCodecException("Offset exceeds size of chunk");
        }
    }
     ...
     * @param data             The input data to calculate the CRC32C checksum of
     * @throws DataCodecException If the calculated and supplied checksums do not match
     */
    static void validateChecksum(int expectedChecksum, ByteBuf data) throws DataCodecException {
     ...
     * @throws DataCodecException If the calculated and supplied checksums do not match
     */
    static void validateChecksum(int expectedChecksum, ByteBuf data, int offset, int length) throws DataCodecException {
        final int actualChecksum = calculateChecksum(data, offset, length);
        if (actualChecksum != expectedChecksum) {
            throw new DataCodecException(
