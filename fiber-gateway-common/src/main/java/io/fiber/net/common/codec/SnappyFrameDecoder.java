 ...
 * use the {@link #SnappyFrameDecoder(boolean, Consumer)} constructor with the argument
 * set to {@code true}.
 */
public class SnappyFrameDecoder extends AbstractedCodec {
     ...
     * {@link #SnappyFrameDecoder(boolean, Consumer)} constructor.
     *
     * @param bufAcceptor xx
     */
    public SnappyFrameDecoder(Consumer<? super ByteBuf> bufAcceptor) {
        this(false, bufAcceptor);
    }
     ...
     *                          {@link DataCodecException} will be thrown
     * @param bufAcceptor       xx
     */
    public SnappyFrameDecoder(boolean validateChecksums, Consumer<? super ByteBuf> bufAcceptor) {
        super(bufAcceptor);
            ...
            switch (chunkType) {
                case STREAM_IDENTIFIER:
                    if (chunkLength != SNAPPY_IDENTIFIER_LEN) {
                        throw new DataCodecException("Unexpected length of stream identifier: " + chunkLength);
                    }
                ...
                case RESERVED_SKIPPABLE:
                    if (!started) {
                        throw new DataCodecException("Received RESERVED_SKIPPABLE tag before STREAM_IDENTIFIER");
                    }
                ...
                case UNCOMPRESSED_DATA:
                    if (!started) {
                        throw new DataCodecException("Received UNCOMPRESSED_DATA tag before STREAM_IDENTIFIER");
                    }
                    if (chunkLength > MAX_UNCOMPRESSED_DATA_SIZE) {
                        throw new DataCodecException("Received UNCOMPRESSED_DATA larger than " +
                    ...
                    bufAcceptor.accept(in.readRetainedSlice(chunkLength - 4));
                    break;
                case COMPRESSED_DATA:
                    if (!started) {
                        throw new DataCodecException("Received COMPRESSED_DATA tag before STREAM_IDENTIFIER");
                    }

                    if (chunkLength > MAX_COMPRESSED_CHUNK_SIZE) {
                        throw new DataCodecException("Received COMPRESSED_DATA that contains" +
                    ...
                    ByteBuf uncompressed = ByteBufAllocator.DEFAULT.buffer(uncompressedSize, MAX_DECOMPRESSED_DATA_SIZE);
                    try {
                        if (validateChecksums) {
                        ...
                        bufAcceptor.accept(uncompressed);
                        uncompressed = null;
                    } finally {
    ...
    private static void checkByte(byte actual, byte expect) throws DataCodecException {
        if (actual != expect) {
            throw new DataCodecException("Unexpected stream identifier contents. Mismatched snappy " +
                    "protocol version?");
        }
    }
